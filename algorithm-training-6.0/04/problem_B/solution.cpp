...
class Solution {
    ...
    public:
        ...
        Solution(int n) : n(n),  is_child(n, false), counts(n, 0), children_of(n) {
        ...
        void PrintCounts() {
            ...
            for ( auto& [name_, id] : names_indexes ) {
                std::cout << name_ << " " << counts[id] << std::endl;
            }
        }
    ...
    private:
        ...
        int calcCounts(int node) {

            
            counts[node] = 0;

            for (int child_id : children_of[node] ) {

                counts[node] += calcCounts(child_id) + 1;
